...
function my_calendar_help() {
	...
	<h1><?php _e( 'How to use Schedule', 'my-calendar' ); ?></h1>
				...
				<div class='mc-support-me'>
					<p>
						<?php
							...
							printf( __( 'Please, consider a <a href="%1$s">donation</a> or a <a href="%2$s">purchase</a> to support Schedule!', 'my-calendar' ), 'https://www.joedolson.com/donate/', 'https://www.joedolson.com/my-calendar/pro/' );
						?>
					</p>
				</div>
				<ul class='list'>
					...
					if ( ! mc_get_uri( 'boolean' ) ) {
						echo '<li>' . __( 'Add the Schedule shortcode (<code>[my_calendar]</code>) to a page.', 'my-calendar' ) . '</li>';
					}
	...
	<div class="ui-sortable meta-box-sortables" id="mc-generator">
		<div class="postbox">
			<h2 id="generator"><?php _e( 'Schedule Shortcode Generator', 'my-calendar' ); ?></h2>
	...
	<div class="ui-sortable meta-box-sortables" id="privacy">
		<div class="postbox">
			...
			<div class="inside">
				<h3><?php _e( 'Data Collection by Schedule', 'my-calendar' ); ?></h3>
				<p>
					<?php _e( 'Schedule collects no personally identifying data.', 'my-calendar' ); ?>
				</p>
				<p>
					<?php _e( 'Schedule Pro, when installed, collects submitter names and email addresses when a public user submits an event from any public event submission form.', 'my-calendar' ); ?>
				</p>
				<h3><?php _e( 'Data Sharing by Schedule', 'my-calendar' ); ?></h3>
				<p>
					<?php _e( 'The names and email addresses of people who author or host events are shared by Schedule as part of the API output, RSS feeds, and iCal formatted event output. This data is sourced from user profiles, and will be destroyed or exported with that information.', 'my-calendar' ); ?>
				</p>
				<p>
					<?php _e( 'Events submitted by public users from any public event submission form using Schedule Pro include names and emails as part of the event data. This data is destroyed when the event is deleted.', 'my-calendar' ); ?>
				</p>
				<h3><?php _e( 'Data Removal in Schedule', 'my-calendar' ); ?></h3>
				<p>
					<?php _e( 'Schedule supports the data export and removal features in WordPress 4.9.6 and later. When a data removal is requested, all events authored using the requested email address will be deleted. All events with that user assigned only as the host will remain, but the host will be changed.', 'my-calendar' ); ?>
				</p>
			</div>
		</div>
	</div>
		...
		<div class="postbox">
			...
			<div class="inside">
				...
				if ( current_user_can( 'administrator' ) ) {
					...
					_e( 'Schedule support requests can only be sent by administrators.', 'my-calendar' );
				}
				?>
			</div>
		</div>
